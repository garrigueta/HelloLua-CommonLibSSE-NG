...
namespace {
    ...
    void InitializeSerialization() {
        ...
        serde->SetSaveCallback(Sample::SKSEManager::OnGameSaved);
        serde->SetRevertCallback(Sample::SKSEManager::OnRevert);
        serde->SetLoadCallback(Sample::SKSEManager::OnGameLoaded);
     ...
     *
     * <p>
     * This sets up the Lua state for our plugin, making it ready to execute Lua scripts.
     * We'll also load any startup scripts that should run when the game starts.
     * </p>
     */
    void InitializeLua() {
        log::trace("Initializing Lua scripting environment...");
        if (auto* luaManager = Sample::LuaManager::GetSingleton()) {
            if (luaManager->Initialize()) {
                log::info("Lua environment initialized successfully");
                
                // Run a test script if it exists
                if (luaManager->ExecuteScript("startup.lua")) {
                    log::info("Successfully executed startup script");
                } else {
                    log::warn("No startup script found or error executing it");
                    
                    // Test by running a simple Lua string
                    luaManager->ExecuteString("Log('Hello from Lua!')");
                }
            } else {
                log::error("Failed to initialize Lua environment");
            }
        }
    }
        ...
        if (!GetMessagingInterface()->RegisterListener([](MessagingInterface::Message* message) {
            switch (message->type) {
                ...
                case MessagingInterface::kDataLoaded: // All ESM/ESL/ESP plugins have loaded, main menu is now active.
                    ...
                    InitializeHooking();
                    InitializeLua(); // Initialize Lua after game data is loaded
                    break;
